...
void fix_imports(iop_library_t *lib)
{
  ...
  for(table = lib->caller; table != NULL; table = table->next)
  {
    ...
    {
      // patch the stub to jump to the address specified in the library
      // export table for "fno"
...
// Directories should be created with execute flag
// otherwise they can't be opened.
...
int hook_ioman(void)
{
  ...
  if (smod_get_mod_by_name(ioman_modname, &info))
  {
    ...
    memcpy(dev_list,
	    (void *) (info.text_start + info.text_size + info.data_size + 0x10),
	    sizeof(iop_device_t *) * 16);

    // steal the original IOMAN's 16 file descriptors
    memcpy(file_table,
	    (void *) (info.text_start + info.text_size + info.data_size + 0x50),
	    sizeof(iop_file_t) * 16);
  }
  else { return(-1); }
