 ...
 *  @{
 *
 *  @name Enabling and Disabling Interrupts
 *  @{
 */
#define DI	DIntr
#define EI	EIntr
...
/** @name Limits
 *  @{
 *  @ingroup thread alarm semaphore
 */
 ...
 *  @{
 *
 *  @name EE Interrupt Controller (INTC) interrupt numbers
 *  @{
 */
enum
{
...
/** @name EE Direct Memory Access Controller (DMAC) interrupt numbers
 *  @{
 */
enum
{
...
/** @name Thread Wait Types
    @{
 */
#define THS_WAIT_NONE	0
#define THS_WAIT_SLEEP	1
#define THS_WAIT_SEMA	2
/** @} */
...
/** @addtogroup semaphore Semaphores
 *  @{
 */
...
//Internal function for reinitializing the TLB, only present in later kernels.
//Please use InitTLB() instead to initialize the TLB with all kernels.
int  _InitTLB(void);
...
//ExecPS2 patch only. Run by _InitSys, Exit, LoadExecPS2, ExecPS2 and ExecOSD
void InitExecPS2(void);
//ExecPS2 + System Configuration patches.
//Please refer to the comments within libosd_full.c
void InitOsd(void);

//Indicates whether the patch is required.
...
// Defined in crt0 - program termination 
